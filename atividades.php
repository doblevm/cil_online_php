...
<script language="javascript">
            function open_win_editar(id) {
                window.open("EditarAtv.php?id="+id, "Executar", "location=1, status=no, scrollbars=no, width=650, height=400");
            }
            function open_win_Excluir(id) {
                window.open("ExcluirAtv.php?id="+id, "Executar", "location=1, status=no, scrollbars=no, width=200, height=110");
            }
...
.subTexto{
  ...
  <body style="background-color: #222E34;"><br/>
	<p class="texto">
		...
		<!----		<option selected="">Selecione</option>---->
			...
			if ($stmt = $con-> prepare($query)) {
			   ...
			   $stmt->close();
				...
				</select> Palavra Chave:<input name="txPesquisa" id="txPesquisa" type="text" style="background: #43494B; border-color:#CECECE;border-radius: 5px 5px 5px 5px; height: 25px;"> <input name="btEnv" type="submit" value=" " style="width: 30px; background: #43494B; border:medium none #43494B; background:url(images/lupa.png); height: 30px;">&nbsp; </form>
		...
		<?php
			...
			//and(!empty($_POST["txArea"]))
				...
				$txArea=$_POST["txArea"];
				$query = "SELECT idAtividade,txAtividade, Periodicidade, HorimetroAlerta,HorimetroUltima,HorimetroProxima, procedimento, Maquina, Horimetro, tbMaquinas.Area FROM tbAtividades inner join tbMaquinas using(idMaquina) WHERE (Area='".$txArea."') and (".$txFiltro." LIKE '%".$txLike."%');";
			}else{
					if(isset($_GET['Area'])){
						$query = "SELECT idAtividade,txAtividade, Periodicidade, HorimetroAlerta,HorimetroUltima,HorimetroProxima, procedimento, Maquina, Horimetro, tbMaquinas.Area FROM tbAtividades inner join tbMaquinas using(idMaquina) WHERE (Area='".$txArea."');";
					}else{
						$query = "SELECT idAtividade,txAtividade, Periodicidade, HorimetroAlerta,HorimetroUltima,HorimetroProxima, procedimento, Maquina, Horimetro, tbMaquinas.Area FROM tbAtividades inner join tbMaquinas using(idMaquina) limit 30;";
		...
		//echo $query ;
			...
			if ($stmt = $con-> prepare($query)) {
			    $stmt->execute();
			    $stmt->bind_result($idAtividade,$txAtividade, $Periodicidade, $HorimetroAlerta, $HorimetroUltima, $HorimetroProxima, $procedimento, $Maquina, $Horimetro, $Area);
			    while ($stmt->fetch()) {
		       ...
		       echo '<tr >';
			    ...
			    echo '  <span class="Conteudo">&nbsp;&nbsp;Proxima Atividade:&nbsp;</span><span class="Conteudo">'.$HorimetroProxima.'&nbsp;<a href="" onclick="open_win_editar('.$idAtividade.');"><img height="50" src="images/Editar.png" width="50" align="right"></a><a href="" onclick="open_win_Excluir('.$idAtividade.');"><img height="50" src="images/lixeira.png"  width="50" align="right"></a>'; 
